...
export class BootWavePainter {
  ...
  render(frameBuffer: OptimizedBuffer, options: GoUpsellArtRenderOptions = {}) {
    ...
    const dt = options.deltaTime ?? 0
    this.elapsed = (this.elapsed + dt) % PERIOD
    this.bootElapsed += dt
    this.rebuildGeometry(frameBuffer, rgb)
    // Boot-wave: suppress the embedded logo glyphs — pure wave only.
    frameBuffer.buffers.char.fill(SPACE)
    frameBuffer.buffers.attributes.fill(0)
    this.drawBackground(frameBuffer, this.elapsed, bootIntensity(this.bootElapsed))
  }
  ...
  private drawBackground(frameBuffer: OptimizedBuffer, t: number, intensity = 1) {
    const buffers = frameBuffer.buffers
    const fg = buffers.fg
    const bg = buffers.bg
    ...
    for (let index = 0; index < distances.length; index++) {
      ...
      const strength = (rawStrength > 1 ? 1 : rawStrength) * 0.7 * intensity
