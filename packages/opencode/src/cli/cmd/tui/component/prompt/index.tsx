  ...
  return (
    <>
      <box ref={(r: BoxRenderable) => (anchor = r)} visible={props.visible !== false} width="100%">
        ...
        <Show when={status().type !== "idle" || warpNotice() || workspaceLabel()}>
          <box width="100%" flexDirection="row" flexShrink={0} paddingLeft={3} paddingRight={3}>
            <Switch>
        ...
        <Show when={!props.hideFooter}>
          ...
          <box flexShrink={0} flexDirection="row" gap={1}>
            <Show when={local.agent.current()} fallback={props.hint ?? <text />}>
              {(agent) => (
                <text>
                  <span style={{ fg: fadeColor(highlight(), modelMetaAlpha()) }}>
