...
class CommonsBgPainter {
  ...
  private drawFlowField(frameBuffer: OptimizedBuffer, cols: number, rows: number) {
    ...
    const cx = cols / 2
    const cy = rows / 2
    const quietRadiusX = Math.max(24, cols * 0.22)
    const quietRadiusY = Math.max(6, rows * 0.20)
    ...
    for (let y = 1; y < rows - 2; y++) {
      for (let x = 0; x < cols; x++) {
        ...
        const keepChance = Math.pow(visibleOpacity, 0.72) * 0.4
...
export function CommonsBg() {
  ...
  const [opacity, setOpacity] = createSignal(0.72)
