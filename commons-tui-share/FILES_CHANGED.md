...
| `packages/opencode/src/cli/cmd/tui/routes/session/sidebar.tsx` | Uses the translucent panel; the inline-default footer wordmark is hardcoded to "Commons" (and `InstallationVersion` is no longer rendered). | **Medium** | The flag ternary is gone from the default — the brand swap is now unconditional. |
| `packages/opencode/src/cli/cmd/tui/feature-plugins/sidebar/footer.tsx` | The internal `sidebar_footer` plugin is the actual winner of the slot. Brand wordmark hardcoded to "Commons"; the `props.api.app.version` suffix is dropped. | **Low** | The plugin is the one that paints the text the user actually sees; the inline default in `sidebar.tsx` is dead unless the plugin is removed. |
